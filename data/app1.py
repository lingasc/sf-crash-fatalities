...
# Custom CSS that's more compatible with Streamlit
st.markdown("""
<style>
/* Main color palette */
:root {
...
/* Base styles */
body {
    font-family: 'Optima', 'Libre Baskerville', serif;
    color: #252D6E;
...
/* Make Streamlit components prettier */
...
st.write("Interactive visualization and predictive analytics for traffic safety")
st.markdown("---")
...
with tab1:
    ...
    with col1:
        ...
        sns.barplot(x='collision_year_clean', y='Count', data=yearly_counts, palette=sunset_melody, ax=ax)
        ax.set_xlabel('Year')
    ...
    with col2:
        ...
        if len(victim_order) > 0:
            # Create the box plot
            fig, ax = plt.subplots(figsize=(10, 6))
            sns.boxplot(x='deceased', y='age', data=filtered_df, order=victim_order, palette=sunset_melody[:len(victim_order)])
